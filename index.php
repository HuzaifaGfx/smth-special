...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && is_logged_in()) {
    ...
    if (is_admin_or_super()) {
        if ($action === 'create_user') {
            ...
            try {
                $pdo->prepare("INSERT INTO users (username, password_hash, role, searches_limit, package_name, package_expiry, created_by) VALUES (?,?,?,?,?,?,?)")
                    ->execute([$new_u, '20202020', $role, $limit, $pkg, $expiry, $creator]);
            ...
            $pdo->prepare("UPDATE users SET package_name = ?, package_expiry = ?, searches_limit = ?, searches_used = 0 WHERE id = ?")
                ->execute([$pkg, $expiry, $limit, $uid]);
...
<head>
    ...
    <style>
        body { background: #f1f5f9; font-family: 'Segoe UI', sans-serif; display: flex; min-height: 100vh; margin: 0; }
        ...
        <?php elseif ($route === 'manage_users'): ?>
            ...
            <div class="card p-4 border-0 shadow-sm overflow-auto">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h5 class="fw-bold mb-0">Manage Accounts</h5>
                ...
                <table class="table align-middle" id="userTable">
                    <thead><tr><th>Username</th><th>Role</th><th>Credits</th><th>Status</th><th>PDF</th><th class="text-center">Actions</th></tr></thead>
                    <tbody>
